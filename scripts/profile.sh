...
if [ -f .env ]; then
    ...
    COMPOSE_PROJECT_NAME=$(grep '^COMPOSE_PROJECT_NAME=' .env | cut -d'=' -f2 | tr -d '"' || echo "drupal")
    # Export variables for use by sourcing scripts
    export DEVELOPMENT_ENVIRONMENT ENABLE_HTTPS URI_SCHEME ENABLE_ACME ACME_EMAIL ACME_URL DOMAIN COMPOSE_PROJECT_NAME
else
