...
if [ -f .env ]; then
    ...
    DOMAIN=$(grep '^DOMAIN=' .env | cut -d'=' -f2 | tr -d '"' || echo "drupal.traefik.me")
    DOCKER_REPOSITORY=$(grep '^DOCKER_REPOSITORY=' .env | cut -d'=' -f2 | tr -d '"' || echo "ghcr.io/libops")
    ...
    export DEVELOPMENT_ENVIRONMENT ENABLE_HTTPS URI_SCHEME ENABLE_ACME ACME_EMAIL DOMAIN COMPOSE_PROJECT_NAME DOCKER_REPOSITORY
else
